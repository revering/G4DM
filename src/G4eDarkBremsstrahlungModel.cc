...
G4double G4eDarkBremsstrahlungModel::ComputeDEDXPerVolume(
...
G4double G4eDarkBremsstrahlungModel::ComputeBremLoss(G4double Z, G4double T, G4double Cut)
...
G4double G4eDarkBremsstrahlungModel::PositronCorrFactorLoss(G4double Z, G4double kineticEnergy, G4double cut)
{
...
G4double G4eDarkBremsstrahlungModel::CrossSectionPerVolume(
                                    const G4Material* material,
                                    const G4ParticleDefinition* p,
                                    G4double kineticEnergy,
                                    G4double cutEnergy,
...
//   G4cout << "Kinetic Energy: " << E0 << " Cross section: " << cross << G4endl;
   ...
   G4double G4eDarkBremsstrahlungModel::PositronCorrFactorSigma( G4double Z,
...
void G4eDarkBremsstrahlungModel::SampleSecondaries(std::vector<G4DynamicParticle*>* vdp, 
                                                ...
                                                G4double maxEnergy)
 {
   ...
   G4double totalEnergy = E0 + electron_mass_c2;
   G4double Mel = 5.1E-04;
   E0 = E0 / 1000.;
...
//       printf( "Ee = %e XAcc = %e ThetaAcc = %e\n ", E0, XAcc, ThetaAcc);
   ...
   E0 = E0*1000.;
   G4double gammaEnergy = XAcc*E0;
   G4ThreeVector newDirection;
   newDirection.set(std::sin(ThetaAcc)*std::cos(PhiAcc),std::sin(ThetaAcc)*std::sin(PhiAcc), std::cos(PhiAcc));
   ...
   G4double totMomentum = sqrt(E0*(totalEnergy + electron_mass_c2));
   G4double dpMomentum = dphoton->GetMomentum(); 

   G4ThreeVector direction = (totMomentum*dp->GetMomentumDirection()
                              - dpMomentum*dphoton->GetMomentumDirection()).unit();
 
   // energy of primary
   G4double finalE = E0 - gammaEnergy;
 ...
 G4double G4eDarkBremsstrahlungModel::SupressionFunction(const G4Material* material,
